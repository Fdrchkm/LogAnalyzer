...
class LogViewer:
    ...
    def generate_report(self):
        ...
        stats = {"ERROR": 0, "FAILED": 0, "WARNING": 0}
        ...
        if len(self.suspicious_events) == 0:
            ...
            self.output.insert(tk.END, f"• Критичних помилок у роботі системи (ERROR): {stats['ERROR']}\n")
            self.output.insert(tk.END, f"• Невдалих операцій або проблем з автентифікацією (FAILED): {stats['FAILED']}\n")
            self.output.insert(tk.END, f"• Потенційно небезпечних подій (WARNING/ALERT): {stats['WARNING']}\n")
    ...
    def export_report(self):
        ...
        if save_path:
            export_to_pdf(self.suspicious_events, save_path)
